...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        LogPrint("antimony", "CreateNewBlock(): total size %u\n", nBlockSize);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("antimonyMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        ...
        return error("antimonyMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("antimonyMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("antimony-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrint("antimony", "Running antimonyMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
